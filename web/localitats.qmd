...
if (any(hi_ha_canvis) &&
  ...
  names(n_canvis) <- c("loc_admin_centre_municipis")
  n_canvis <- n_canvis[n_canvis > 0]

  missatge <- paste0(
    "Hi ha ", sum(n_canvis),
    " objectes amb canvis a [localitats](https://osm-catalan.github.io/monitorOSM/web/localitats.html).\n",
    "```", paste(capture.output(print(as.data.frame(n_canvis))), collapse = "```\n```"), "```"
  )
